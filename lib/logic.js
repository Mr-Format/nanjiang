...
new Promise(function(resolved, rejected) {
    $.ajax({
        ...
        function stickEffect(pinchOne) {
            ...
            if (left > 0 || top > 0 || skWidth + left < winWidth || skHeight + top + bannerHeight < winHeight) {
                //console.log("超出屏幕范围");
                if (left > 0) {
                    setLeft = 0;
                }
                ...
                if (skHeight + top + bannerHeight < winHeight) {
                    setTop = -(skHeight - winHeight + bannerHeight);
                }
