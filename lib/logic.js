...
//外部容器滚动
  ...
  //下面两个是20190620增加的
    ...
    function stickEffect(pinchOne) {
        ...
        // alert(left)
        if (left > (skWidth - imgWidth)/2 || top > (skHeight-top)/2 || skWidth + left < winWidth || skHeight + top  < winHeight) {
            //console.log("超出屏幕范围");
            if (left > (skWidth - imgWidth)/2) {
                setLeft = (skWidth - imgWidth)/2;
            }
            if (top > (skHeight-top)/2) {
                setTop = (skHeight-top)/2;
            }
